...
def author_exists(db: Session, name: str = None, author_id: int = None) -> bool:
...
def get_books(
    db: Session, skip: int = 0, limit: int = 10, author_id: int | None = None
):
    stmt = select(models.Book).offset(skip).limit(limit)

    if author_id:
        stmt = stmt.join(models.Author).where(models.Author.id == author_id)
