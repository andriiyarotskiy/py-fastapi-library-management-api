from typing import List

from fastapi import FastAPI, Depends, HTTPException
from sqlalchemy.orm import Session

import crud
import schemas
from database import engine
from models import Base

# Create tables in the database
Base.metadata.create_all(bind=engine)


def get_session():
    with Session(engine) as session:
        yield session


app = FastAPI()


@app.get("/authors/", response_model=List[schemas.Author])
def author_list(
        skip: int = 0,
        limit: int = 10,
        db: Session = Depends(get_session)
):
    return crud.get_authors(db=db, skip=skip, limit=limit)


@app.post("/authors/", response_model=schemas.Author)
def create_author(
        author: schemas.AuthorCreate,
        db: Session = Depends(get_session)
):
    if crud.author_exists(db, author.name):
        raise HTTPException(status_code=422, detail="Author already exists")
    return crud.create_author(db=db, author=author)


@app.get("/authors/{author_id}", response_model=schemas.Author)
def retrieve_author(author_id: int, db: Session = Depends(get_session)):
    author = crud.get_author(db=db, author_id=author_id)
    if not author:
        raise HTTPException(status_code=404, detail="Author does not exist")
    return author


@app.get("/books/", response_model=List[schemas.Book])
def book_list(
        skip: int = 0,
        limit: int = 10,
        db: Session = Depends(get_session),
        author_id: int | None = None,
):
    return crud.get_books(db=db, skip=skip, limit=limit, author_id=author_id)


@app.post("/books/", response_model=schemas.Book)
def create_book(book: schemas.BookCreate, db: Session = Depends(get_session)):
    author = crud.get_author(db=db, author_id=book.author_id)
    if not author:
        raise HTTPException(status_code=404, detail="Author not found")
    return crud.create_book(db=db, book=book)
